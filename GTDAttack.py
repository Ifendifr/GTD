...
X = pd.get_dummies(gtd_df[['country_txt', 'region_txt', 'weaptype1_txt', 'nkill']],
                   drop_first = True)

# Labels
y = gtd_df['attacktype1_txt']
...
test = pd.DataFrame(X_test.head())
ytest = pd.DataFrame(y_test.head())
res = pd.concat([test, ytest], axis=1)
res.to_csv("attack_predict.csv", sep = ",")
print(pred_lables1.head())
